...
public class Dewikify extends AbstractEditAction {
	...
	public Dewikify(BasicEdit b1, BasicEdit b2) {
		this.be = new BasicEdit[2];
		this.be[0] = b1;
		this.be[1] = b2;
	}
	
	public Dewikify(BasicEdit[] be) {
		this.be = be;
	}
					...
					for (int j = 0; j < content.length; j++)
					{
							...
							content[j].kind == MediawikiScannerConstants.ITALIC)
						{
							...
							if (remainder.length > 0)
							{
								...
								editList.add(new Deletion(ie.getPos()+j+1, remainder));
							}
							break;
						}
					}
					ret.add(new Dewikify(b1, ie));
					i++;
				}
				else
				{
					ret.add(new Dewikify(b1, b2));
					i++;
				}
			}
			else {
